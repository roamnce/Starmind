...
class InteractiveCanvasViewer extends StatefulWidget {
  ...
  /// Called when the user begins an interaction.
  final GestureScaleStartCallback? onInteractionStart;

  /// Called when the user updates an interaction.
...
class _InteractiveCanvasViewerState extends State<InteractiveCanvasViewer>
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    // If the corrected translation doesn't fit in either direction, check if
    // elastic boundary should be applied. This happens when the viewport is
    // larger than the entire boundary (PDF smaller than viewport).
    if (offendingCorrectedDistance.dx != 0.0 &&
        offendingCorrectedDistance.dy != 0.0) {
      // Check if PDF is smaller than viewport in any dimension - use elastic boundary
      final bool pdfSmallerThanViewport =
          _boundaryRect.width < _viewport.width ||
          _boundaryRect.height < _viewport.height;

      if (pdfSmallerThanViewport) {
        // Apply elastic boundary constraint to allow elastic panning
        final Offset currentTranslation = _getMatrixTranslation(nextMatrix);
        final Offset constrainedTranslation = ElasticBoundary.constrain(
          currentTranslation,
          _boundaryRect,
          _viewport,
        );
      ...
      // Hard boundary: PDF is larger than viewport but still can't fit
      return matrix.clone();
    }
