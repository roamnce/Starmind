...
class GuruMindDataConverter {
  ...
  Note noteFromDocument(Map<String, dynamic> document, {required String topicId}) {
  ...
  Map<String, dynamic> _normalizeTopicMap(Map<String, dynamic> document) {
    ...
    return {
      'id': document['id'] ?? document['uuid'] ?? '0-${DateTime.now().microsecondsSinceEpoch}',
      'title': document['title'] ?? document['name'] ?? 'Imported Mindmap',
      'author': document['author'],
      'pdf_ids': _join(document['pdf_ids'] ?? document['pdfIds']),
      'root_note_ids': _joinIds(document['root_note_ids'] ?? document['rootNoteIds'] ?? document['root_ids'] ?? document['rootNodes']),
  ...
  Map<String, dynamic> _normalizeNoteMap(Map<String, dynamic> document, {required String topicId}) {
    final now = DateTime.now().toIso8601String();
    final title = document['title'] ?? document['name'] ?? document['text'] ?? 'Imported Note';
    final content = document['content_json'] ??
        (document['content'] is Map ? NoteContent.fromJson(Map<String, dynamic>.from(document['content'] as Map)).toJson() : null);
    return {
      'id': _toStarmindNoteId(document['id'] ?? document['uuid'] ?? '${DateTime.now().microsecondsSinceEpoch}'),
      'topic_id': document['topic_id'] ?? document['topicId'] ?? topicId,
      'parent_id': document['parent_id'] ?? document['parentId'],
      'title': title,
      'content_json': content is String ? content : content == null ? null : _json(content),
      'child_ids': _joinIds(document['child_ids'] ?? document['childIds'] ?? document['children']),
      ...
      'highlight_style': document['highlight_style'] ?? document['highlightStyle'],
      ...
      'updated_at': document['updated_at'] ?? document['updatedAt'] ?? now,
      'sync_version': document['sync_version'] ?? 0,
      'layout_style': document['layout_style'] ?? document['layoutStyle'] ?? 'normal',
    };
  }

  String _toStarmindNoteId(Object? value) {
    final id = value?.toString() ?? '';
    if (id.startsWith('0-') || id.startsWith('1-') || id.startsWith('2-')) return id;
    return '1-$id';
  }

  String _toGuruMindNodeId(String id) => id.startsWith('1-') ? id.substring(2) : id;
  ...
  String? _joinIds(Object? value) {
    ...
    if (value is Iterable) return value.whereType<Object>().map(_toStarmindNoteId).join('|');
  ...
  String? _join(Object? value) {
    ...
    if (value is Iterable) return value.whereType<Object>().map((item) => item.toString()).join('|');
